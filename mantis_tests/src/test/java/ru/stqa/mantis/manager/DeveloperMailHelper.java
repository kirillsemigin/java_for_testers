...
public class DeveloperMailHelper extends HelperBase{
    ...
    public DeveloperMailUser addUser() {
        ...
        Request request = new Request.Builder()
                .url("https://developermail.com/api/v1/mailbox")
                .put(body)
                .build();
    ...
    public void deleteUser(DeveloperMailUser user) {
        Request request = new Request.Builder()
                .url(String.format("https://developermail.com/api/v1/mailbox/%s", user.name()))
    ...
    public String receive(DeveloperMailUser user, Duration duration) {
        ...
        while (System.currentTimeMillis() < start + duration.toMillis()) {
            try {
                var text1 = get(String.format("https://developermail.com/api/v1/mailbox/%s", user.name()), user.token());
                ...
                if (response1.result().size() > 0 ) {
                    var text2 = get(String.format("https://developermail.com/api/v1/mailbox/%s/messages/%s",
